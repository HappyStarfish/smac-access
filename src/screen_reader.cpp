...
void sr_log(const char* msg) { sr_write_log(sr_get_session_logfile(), msg); }
...
void sr_hook_log(const char* msg) { sr_write_log("access_hooks.log", msg); }
...
static int sr_install_popup_hooks(uint8_t* tramp_slot) {
    ...
    // tech_achieved + planetfall hooks moved to sr_misc_hooks.cpp
...
bool sr_install_text_hooks() {
    ...
    // Install misc hooks (tech_achieved, planetfall, load/save, number input)
    hooked += sr_install_misc_hooks(slot);

    // Install council hooks (call_council, gfx_event_loop, buy_votes)
    hooked += sr_install_council_hooks(slot);
